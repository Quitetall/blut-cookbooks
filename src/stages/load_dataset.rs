        ...
        match (&args.path, &args.hf_name) {
            (Some(p), None) => {
                ...
                if p.components().any(|c| matches!(c, std::path::Component::ParentDir)) {
                    return Err(StageError::BadInput(format!(
                        "path '{}' contains '..' — refusing", p.display()
                    )));
                }
                if !p.exists() {
                    return Err(StageError::BadInput(format!(
                        "dataset path not found: {}", p.display()
                    )));
                }
            ...
            (None, Some(hf_name)) => {
                ...
                let status = cmd.status().map_err(|e| StageError::Backend(
                    anyhow::anyhow!("failed to run blut_core.load_dataset: {}", e)
                ))?;
                if !status.success() {
                    return Err(StageError::Backend(
                        anyhow::anyhow!("blut_core.load_dataset exited with {}", status)
                    ));
                }
                if !output_path.exists() {
                    return Err(StageError::Backend(
                        anyhow::anyhow!("blut_core.load_dataset did not produce output")
                    ));
                }
